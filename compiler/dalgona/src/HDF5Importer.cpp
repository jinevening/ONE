...
DataType toInternalDtype(const H5::DataType &h5_type)
{
  ...
  if (h5_type == H5::PredType::STD_I16BE || h5_type == H5::PredType::STD_I16LE)
  {
    return DataType::S16;
  }
...
void readTensorData(H5::DataSet &tensor, int16_t *buffer)
{
  tensor.read(buffer, H5::PredType::NATIVE_INT16);
}

void readTensorData(H5::DataSet &tensor, int32_t *buffer)
{
  tensor.read(buffer, H5::PredType::NATIVE_INT32);
}

void readTensorData(H5::DataSet &tensor, int64_t *buffer)
{
  tensor.read(buffer, H5::PredType::NATIVE_INT64);
}
  ...
  *shape = toInternalShape(tensor_shape);
  ...
  switch (*dtype)
  {
    case DataType::FLOAT32:
      ...
      break;
    case DataType::S16:
      readTensorData(tensor, static_cast<int16_t *>(buffer));
      break;
    case DataType::S32:
