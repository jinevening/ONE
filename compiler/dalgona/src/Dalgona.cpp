  ...
  for (int32_t record_idx = 0; record_idx < num_records; record_idx++)
  {
    ...
    for (int32_t input_idx = 0; input_idx < num_inputs; input_idx++)
    {
      ...
      if (!is_raw_data)
      {
        ...
        // Automatic quantization from fp32 input to uint8
        if (importer.dtype(0, 0) == DataType::FLOAT32 && input_node->dtype() == DataType::U8)
        {
          float scale = input_node->quantparam()->scale[0];
          int64_t zp = input_node->quantparam()->zerop[0];
          std::vector<float> float_input(getTensorSize(input_node));
          importer.readTensor(record_idx, input_idx, &dtype, &shape, float_input.data());
          for (int i = 0; i < float_input.size(); i++)
          {
            int32_t data = std::round(float_input[i] / scale) + zp;
            data = std::max(0, std::min(255, data));
            if (data < 0 || data > 255)
              throw std::runtime_error("Auto-quantized input data out of bound: " +
                                       std::to_string(data));
